...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
static enum usb_otg_state otg_state_cache = OTG_STATE_UNDEFINED;
static int usbcompswitch = 0;
#endif
/* NTGRSTOP */
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
int dwc3_get_usbcompswitch(void)
{
	return usbcompswitch;
}

void dwc3_set_usbcompswitch(int val)
{
	usbcompswitch = val;
}

static ssize_t show_usbcompswitch(struct device *dev, struct device_attribute *attr,
			   char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n",usbcompswitch);
}
static DEVICE_ATTR(usbcompswitch, S_IRUSR, show_usbcompswitch, NULL);

/**
 * dwc3_usbcompswitch_sysfs_create_file: initializes the attribute interface
 * @dev: device
 *
 * This function returns an error code
 */
__maybe_unused static int dwc3_usbcompswitch_sysfs_create_file(struct device *dev)
{
	int retval = 0;

	if (dev == NULL)
		return -EINVAL;
	/* Create sysfs to /sys/devices/8a00000.ssusb/8a00000.dwc3/ */
	retval = device_create_file(dev, &dev_attr_usbcompswitch);

	return retval;
}

/**
 * dwc3_usbcompswitch_sysfs_remove_file: destroys the attribute interface
 * @dev: device
 *
 * This function returns an error code
 */
__maybe_unused static int dwc3_usbcompswitch_sysfs_remove_file(struct device *dev)
{
	if (dev == NULL)
		return -EINVAL;

	device_remove_file(dev, &dev_attr_usbcompswitch);
	return 0;
}
#endif
/* NTGRSTOP */
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
/* case 03465869 APPS Crash - Internal error: (FSR = 0x817) (PC = memset+0xd0/0xe0) */
	int ret;
#endif /* NETGEAR */
/* NTGRSTOP */
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
	dwc3_usbcompswitch_sysfs_create_file(dwc->dev);
#endif
/* NTGRSTOP */
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
	dwc3_usbcompswitch_sysfs_remove_file(dwc->dev);
#endif
/* NTGRSTOP */
...
/* NTGRSTART */
	...
	if (otg_state_cache != mdwc->otg_state) {
		dev_info(mdwc->dev, "OTG state changed from %s to %s\n", usb_otg_state_string(otg_state_cache), usb_otg_state_string(mdwc->otg_state));

		if (mdwc->otg_state == OTG_STATE_A_HOST && usbcompswitch == 1)
		{
			usbcompswitch = 0;
		}
		otg_state_cache = mdwc->otg_state;
		kobject_uevent(&mdwc->dev->kobj, KOBJ_CHANGE);
	}
#endif
/* NTGRSTOP */
