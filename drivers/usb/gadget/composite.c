...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
			pr_info("USB_DT_STRING, 0x%x", w_value);

			if (sierra_smem_get_rndis6_mode() == 1 && 
				cdev->desc.idProduct == 0x68e1 && 
				((w_value & 0xff) == 6 || (w_value & 0xff) == 7)
				&& dwc3_get_usbcompswitch() == 0) 
			{
				pr_info("Set usbcompswitch to 1");
   				dwc3_set_usbcompswitch(1);
			}
#endif
/* NTGRSTOP */
