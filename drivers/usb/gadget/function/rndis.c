...
static unsigned int swi_mtu = 0;
module_param(swi_mtu, uint, (S_IRUGO | S_IWUSR));
MODULE_PARM_DESC(swi_mtu,
		"MTU size of RNDIS_OID_GEN_MAXIMUM_FRAME_SIZE msg");
#endif /* NETGEAR */
/* NTGRSTOP */
...
#else
			/* Keep the check of rndis_per_dev_params[configNr].dev above */
			pr_info("%s swi_mtu %d", __func__, swi_mtu);
			/* Use Netgear MTU size for response only, 
			* per RFC791, Every internet module must be able to forward a datagram of
			68 octets without further fragmentation.  This is because an internet
			header may be up to 60 octets, and the minimum fragment is 8 octets. */
			if (swi_mtu >= 68)
			{
				*outbuf = cpu_to_le32(swi_mtu);
			}
			else
			{
				*outbuf = cpu_to_le32(
...
{
	return rndis_per_dev_params[configNr].media_state;
}
EXPORT_SYMBOL_GPL(rndis_state);
#endif
/* NTGRSTOP */
