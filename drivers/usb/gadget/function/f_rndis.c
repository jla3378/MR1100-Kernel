...
static bool rndis_initialized = false;
static unsigned int rndis_control = 0;
int rndis_control_set_uint(const char *val, const struct kernel_param *kp);
const struct kernel_param_ops rndis_param_ops = 
{
	.set = &rndis_control_set_uint,  // Use our setter ...
	.get = &param_get_uint,          // Use standard getter
};
module_param_cb(rndis_control, &rndis_param_ops, &rndis_control, (S_IRUGO | S_IWUSR));
MODULE_PARM_DESC(rndis_control,
		"Switch the RNDIS media sense from swiapp");

unsigned int swi_rndis = 0;
module_param(swi_rndis, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(swi_rndis,
		"Control the RNDIS media sense at boot up");
#endif /* NETGEAR */
/* NTGRSTOP */
...
int rndis_control_set_uint(const char *val, const struct kernel_param *kp)
{
	bool* pvalue = kp->arg; // Pointer to actual parameter variable.
	int res = -1;           // Use helper for write variable

	if (!rndis_initialized) {
		pr_err("%s: RNDIS not initialized", __func__);
		return res;
	}

	if ((sscanf(val, "%u", &rndis_control) != 1) || (rndis_control > 1)) {
		pr_err("%s <1|0>: disconnect|connect");
		return res;
	}
	pr_info("%s rndis_control=%d current state=%d\n", __func__, rndis_control, rndis_state(0));

	/* Only have rndis0 supported for now, hardcode config to 0 */
	if (rndis_state(0) != rndis_control)
	{
		if (rndis_control)
		{
			rndis_signal_disconnect(0);
		}
		else
		{
			rndis_signal_connect(0);
		}
	}

	/* Use support function for read later */
	res = param_set_uint(val, kp);
	if(res)
	{
		pr_err("%s param_set_uint error %d\n", __func__, res);
	}
