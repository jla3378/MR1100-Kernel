...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
const static u8 ntgr_ethaddr[ETH_ALEN]={0x00,0xA0,0xC6,0x00,0x00,0x00};
#endif
/* NTGRSTOP */
...
#else
	/* It is for USB TE enable start up state */
	if (swi_rndis)
	{
		rndis_signal_connect(rndis->config);
		pr_info("%s: rndis config = %d connect\n", __func__, rndis->config);
	}
	else
	{
		rndis_signal_disconnect(rndis->config);
		pr_info("%s: rndis config = %d disconnect\n", __func__, rndis->config);
	}
#endif
/* NTGRSTOP */
}
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
	/* QTI case 03117527 remote wakeup disable patch */
	struct f_gsi *gsi = func_to_gsi(f);
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
	if (sierra_smem_get_rndis6_mode() == 1)
	{
		/* Note, USB COMP switch only happens for PID 0x68E1 */
       	if (dwc3_get_usbcompswitch() == 1)
		{
			rndis_gsi_iad_descriptor.bFunctionClass = USB_CLASS_WIRELESS_CONTROLLER; 
			rndis_gsi_iad_descriptor.bFunctionSubClass = 0x01; 
			rndis_gsi_iad_descriptor.bFunctionProtocol = 0x03; 
			rndis_gsi_control_intf.bInterfaceClass = USB_CLASS_WIRELESS_CONTROLLER; 
		...
		else
		{
			...
			rndis_gsi_iad_descriptor.bFunctionSubClass = 0x4; 
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
		pr_info("Getting random host_ethaddr=%pM, device_ethaddr=%pM",
			gsi->d_port.ipa_init_params.host_ethaddr,
			gsi->d_port.ipa_init_params.device_ethaddr);
		memcpy(&gsi->d_port.ipa_init_params.host_ethaddr[0],&ntgr_ethaddr[0],
			ETH_ALEN);
		pr_info("Setting RNDIS host_ethaddr=%pM, device_ethaddr=%pM",
			gsi->d_port.ipa_init_params.host_ethaddr,
			gsi->d_port.ipa_init_params.device_ethaddr);
#endif
/* NTGRSTOP */
...
/* NTGRSTART */
		...
		memcpy(&gsi->d_port.ipa_init_params.host_ethaddr[0],&ntgr_ethaddr[0],
			ETH_ALEN);
		pr_info("Setting ECM host_ethaddr=%pM, device_ethaddr=%pM",
			gsi->d_port.ipa_init_params.host_ethaddr,
			gsi->d_port.ipa_init_params.device_ethaddr);
#endif
/* NTGRSTOP */
