...
#define NETGEAR_HVDCP_SUPPORT

/* Custom feature mask bits for Charger Driver */
#define BYCUST_FET_MASK         0x0001    /* FET control mask (0=try to open FET, 1=charger manages) */
#define BYCUST_RECHG_MASK       0x0002    /* Recharge level (0=normal level, 1=hot level) */
#define BYCUST_QCHARGE_MASK     0x0004    /* QuickCharge mask (0=disable or normal adapter present, 1=enable or QC adapter present) */
#define BYCUST_5V_ADAPTER_MASK  0x0010    /* 5V adapter (1=yes, 0=higher than 5v) */

/* input current limits */
#define INCURR_LIMIT_DEFAULT     500   /* mA (must be USB spec) */
#define INCURR_LIMIT_PRE_EN      330   /* mA (limit just before enable charging) */
#define INCURR_LIMIT_AC_SAG      330   /* mA (wall adapter with voltage sags below UVLO 3.6v) */
#define INCURR_LIMIT_AC          960   /* mA (wall adapter) */
#define INCURR_LIMIT_AC2        1800   /* mA (wall adapter, 2.0 Amps) */
#define INCURR_LIMIT_QUICK_5V   1801   /* mA (wall adapter, QuickCharge, 5V) */
#define INCURR_LIMIT_QUICK_9V   1100   /* mA (wall adapter, QuickCharge, 9V) */

/* output current limits */
#define OUTCURR_LIMIT_USB         840   /* mA */
#define OUTCURR_LIMIT_AC          1200  /* mA (wall adapter, normal 1 Amp) */
#define OUTCURR_LIMIT_AC_QCHARGE  1440  /* mA (wall adapter, 2 Amp or QuickCharge ) */
#endif
/* NTGRSTOP */
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
#define CFG_B_REG			0x0B    /* DCIN config */
#endif
/* NTGRSTOP */
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
#define QUICKCHARGE_MASK		HVDCP_EN_BIT
#define QUICKCHARGE_ENABLE		HVDCP_EN_BIT
#define QUICKCHARGE_DISABLE		0
#endif
/* NTGRSTOP */
...
#define STANDBY_VCHG_EN    BIT(2)
#define STANDBY_VCHG_MASK  BIT(2)
#endif
/* NTGRSTOP */
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
#define BATT_FET_OVERRIDE_EN   BIT(7)  /* 1=enable FET override, 0=no override */
#endif
/* NTGRSTOP */
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
#define HOT_SOFT_LIMIT_MASK     HOT_SOFT_CURRENT_COMP_EN_BIT
#define HOT_SOFT_LIMIT_ENABLE   HOT_SOFT_CURRENT_COMP_EN_BIT
#endif
/* NTGRSTOP */
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
#define CFG_JEITA_LIMITS_REG		CFG_1B_REG
#define JEITA_LIMITS (COLD_HARD_n1C | COLD_SOFT_10C | HOT_SOFT_42C | HOT_HARD_52C)

/* Hot-Hard temperatures are when charging stops */
#define HOT_HARD_63C				(BIT(5) | BIT(4))
#define HOT_HARD_58C				BIT(5)
#define HOT_HARD_52C				BIT(4)
#define HOT_HARD_47C				0

/* Hot-Soft temperatures are when Charger reduces voltage */
#define HOT_SOFT_52C				(BIT(1) | BIT(0))
#define HOT_SOFT_47C				BIT(1)
#define HOT_SOFT_42C				BIT(0)
#define HOT_SOFT_36C				0

/* Cold-Soft temperatures are when Charger reduces voltage */
#define COLD_SOFT_n6C				(BIT(3) | BIT(2))   /* -6 degrees */
#define COLD_SOFT_n1C				BIT(3)              /* -1 degrees */
#define COLD_SOFT_5C				BIT(2)              /* +5 degrees */
#define COLD_SOFT_10C				0                   /* +10 degrees */

/* Cold-Hard temperatures are when charging stops */
#define COLD_HARD_n11C			(BIT(7) | BIT(6))   /* -11 degrees */
#define COLD_HARD_n6C				BIT(7)              /* -6 degrees */
#define COLD_HARD_n1C				BIT(6)              /* -1 degrees */
#define COLD_HARD_5C				0                   /* +5 degrees */


/* When battery is hot, charger's output voltage is reduced based on 4.2v or 4.35v battery */
#define HOT_COMPENSATION_REG		0x1D
#define HOT_CURRENT_LIMIT_MASK  (BIT(7) | BIT(6))
#define HOT_CURRENT_LIMIT_250MA 0x00
#endif
/* NTGRSTOP */
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
#define FET_EDIT_MASK			(BIT(2) | BIT(7))
#define FET_OPEN_MASK			(BIT(2) | BIT(7))
#define FET_CHARGER_MASK		(BIT(2))
#define FET_OVERRIDE_MASK		(BIT(7))
#endif
/* NTGRSTOP */
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
#define USBSUSPEND_BIT			BIT(3)

/* Input current limit register (both USBIN and DCIN wireless charging) */
#define IN_LIMIT_STATUS_REG		0x46   /* status register */
#define IN_LIMIT_MASK			0x1F

/* Charger's voltage registers */
#define FLOATV_STATUS_REG		0x48   /* status register */
#define FLOATV_MASK				0x3F

#define FLOATV_MIN				3600   /* millvolts */
#define FLOATV_MIN_STEP			0x05   /* register values 0x00 to 0x05 are FLOATV_MIN */ 
#define FLOATV_MAX				4500   /* millvolts */
#define FLOATV_MAX_STEP			0x33   /* register values 0x33 to 0x3F are FLOATV_MAX */
#define FLOATV_STEP				  20   /* millivolts per step */
#endif
/* NTGRSTOP */
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
#define STATUS_HVDCP_REG		0x4D
#define HVDCP_DETECT_BIT		BIT(4)
#define HVDCP_9V_DETECT_BIT		BIT(1)
#define HVDCP_5V_DETECT_BIT		BIT(0)
#endif
/* NTGRSTOP */

/* NTGRSTART */
#if !defined(CONFIG_NETGEAR) || !defined(NETGEAR_HVDCP_SUPPORT)
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
#define STATUS_USBIN_MASK  (USBIN_LV | USBIN_UNREG | USBIN_9V)
#endif
/* NTGRSTOP */
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
#define IRQ_D_BATTOVP_BIT		BIT(7)	/* battery voltage is higher than charger's */
#endif
/* NTGRSTOP */
...
enum {
	...
	CURRENT = BIT(2),
/* NTGRSTART */
/* DCIN_DISABLE bit is to disable the wireless charging */
#ifdef CONFIG_NETGEAR
	DCIN_DISABLE = BIT(3),
#endif
/* NTGRSTOP */
...
#ifdef CONFIG_NETGEAR
	/*
	 * true  = Qualcomm health property (temperature-state only).
	 * false = Netgear health property (fault-state only).
	 */
	bool				prop_health_temp;
#endif
/* NTGRSTOP */
...
/* forward references */
static int smb135x_get_prop_volt_now(struct smb135x_chg *chip);
...
/*
 * Purpose: Test if Charger's FET is open.
 *
 * Parms:   (IN) chip       - pointer to Charger structure
 *          (IN) regCmd41   - Command register 0x41
 *          (IN) reg4A      - Status register 0x4A
 *  
 * Return:  true, if FET open
 *          false, if FET closed
 *
 * NOTE:    We infer FET open/closed because there's no status bit telling us directly.
 *
 *          The FET will automatically close if the input-power is weak/missing/disabled 
 *          so the battery [helps] supply power to Vsys.  Closing the FET eliminates a 0.7v drop
 *          across a diode that's in parallel with the FET.
 * 
 */
static bool is_charge_FET_open( struct smb135x_chg *chip,
                                u8 regCmd41, u8 reg4A)
{
	/* IF the FET is forced open */
	if (regCmd41 & FET_OVERRIDE_MASK) {
		return true;
	}

	/* FET is closed during discharging */
	if (reg4A & BATT_NET_CHG_CURRENT_BIT) {
		/* Discharging can occur when Charger is Enabled or Disabled regardless of battery-only/USB/AC */
		return false;
	}

	/* IF plugged into AC or USB */ 
	if (chip->usb_present) {

		/* FET is closed during charging */
		u8 field = (reg4A & CHG_TYPE_MASK) >> CHG_TYPE_SHIFT;
		if (field != BATT_NOT_CHG_VAL) {
			return false;
		}
	}

	/*
	 * FET is open here for one of the following:
	 *
	 *   a) Input-power is sufficent for Vsys so battery is not used.
	 *   b) Issue 11 in Qualcomm doc 80-NJ318-4
	 *       (Need low current limit when USB insertion)
	 *   c) Issue 12 in Qualcomm doc 80-NJ318-4
	 *       (Need Standby VCHG disabled before OTG disable)
	 */
...
#else
{
	...
	/*
	 * For h/w testing, engineers need to verify if the FET is open or closed.
	 * While Qualcomm returns FET-open as POWER_SUPPLY_STATUS_DISCHARGING,
	 * the open status can be hidden if the battery is full so the full test
	 * is moved after the FET-open test.   
	 */
	rc = smb135x_read(chip, STATUS_4_REG, &reg);
	if (rc < 0) {
		dev_err(chip->dev, "Unable to read STATUS_4_REG rc = %d\n", rc);
	...
	rc = smb135x_read(chip, CMD_INPUT_LIMIT, &regcmdlim);
	if (rc < 0) {
		dev_err(chip->dev, "Unable to read CMD_INPUT_LIMIT rc = %d\n", rc);
		return POWER_SUPPLY_STATUS_UNKNOWN;
	}

	/* precedence to POWER_SUPPLY_STATUS_DISCHARGING so we can tell when the FET is open */
	if (is_charge_FET_open( chip, regcmdlim, reg)) {
		status = POWER_SUPPLY_STATUS_DISCHARGING;
		goto out;
	}

	rc = smb135x_read(chip, STATUS_1_REG, &regsusp);
	if (rc < 0) {
		dev_err(chip->dev, "Unable to read STATUS_1_REG rc = %d\n", rc);
		return POWER_SUPPLY_STATUS_UNKNOWN;
	}

	/* IF the USB Input is suspended due to error or disable */
	if (regsusp & USBSUSPEND_BIT)
		return POWER_SUPPLY_STATUS_NOT_CHARGING;

	/* full-battery test is lower priority than FET open */
	if (chip->chg_done_batt_full)
		return POWER_SUPPLY_STATUS_FULL;
}
#endif
...
/*************
 *
 * Name:     smb135x_get_prop_charger_faults
 *
 * Purpose:  Get state of Charger faults
 *
 * Parms:    (IN) chip  - pointer to driver structure
 *
 * Return:   POWER_SUPPLY_HEALTH_GOOD 
 *           POWER_SUPPLY_HEALTH_OVERVOLTAGE
 *           POWER_SUPPLY_HEALTH_SAFETY_TIMER_EXPIRE
 *           or
 *           POWER_SUPPLY_HEALTH_UNKNOWN, if cannot read status/fault
 *
 **************/
static int smb135x_get_prop_charger_faults(struct smb135x_chg *chip)
{
	int rc;
	u8  reg;

	/* IF cannot read Battery status */
	...
	if (reg & IRQ_D_BATTOVP_BIT)
	{
		return POWER_SUPPLY_HEALTH_OVERVOLTAGE;
	}

	if (reg & IRQ_D_TIMEOUT_BIT)
	{
...
/*************
 *
 * Name:     smb135x_get_prop_curr_termination
 *
 * Purpose:  Get Charger's current termination
 *
 * Parms:    (IN) chip  - pointer to driver structure
 *
 * Return:   milliamps
 ...
 *
 **************/
static int smb135x_get_prop_curr_termination(struct smb135x_chg *chip)
{
  int rc;
  u8  reg;

  /* IF cannot read Charger config */
  rc = smb135x_read(chip, CFG_3_REG, &reg);
  if (rc < 0)
  {
    return 0;
  }
  ...
  switch( reg)
  {
    case CHG_ITERM_50MA:     return  50;
    case CHG_ITERM_100MA:    return 100;
    case CHG_ITERM_150MA:    return 150;
    case CHG_ITERM_200MA:    return 200;
    case CHG_ITERM_250MA:    return 250;
    case CHG_ITERM_300MA:    return 300;
    case CHG_ITERM_500MA:    return 500;
    case CHG_ITERM_600MA:    return 600;
...
/*************
 *
 * Name:     smb135x_get_otg_control
 *
 * Purpose:  Get Charger's OTG mode
 *
 * Parms:    (IN) chip  - pointer to driver structure
 *
 * Return:   1= OTG hosting enabled
 *           0= not hosting (either we're a normal USB slave or OTG slave)
 *
 **************/
static int smb135x_get_otg_control( struct smb135x_chg *chip)
{
	return smb135x_chg_otg_regulator_is_enable( chip->otg_vreg.rdev);
}

/*************
 *
 * Name:     smb135x_get_charge_control_mask
 *
 * Purpose:  Get custom charge control
 *
 * Parms:    (IN) chip  - pointer to driver structure
 *
 * Return:   BYCUST_QCHARGE_MASK, if QuickCharge adapter is detected
 *           0, if not
 *
 * Note:     Other control bit-masks are not returned at this time.
 *
 **************/
static int smb135x_get_charge_control_mask( struct smb135x_chg *chip)
{
	int rc;
	int acstatus = 0;
	u8 reg;
	...
	/* get HVDCP status */
	rc = smb135x_read(chip, STATUS_HVDCP_REG, &reg);
	if (rc < 0) {
		dev_err(chip->dev, "Cannot read HVDCP rc = %d\n", rc);
		return 0;
	}

	/* IF QuickCharge */
	if (reg & HVDCP_DETECT_BIT)
	{
		acstatus |= BYCUST_QCHARGE_MASK;

		/* IF 5v QuickCharge */
		if (reg & HVDCP_5V_DETECT_BIT)
		{
			acstatus |= BYCUST_5V_ADAPTER_MASK;
		}

		/* Don't need to check other registers if quickcharge detected */
		return acstatus;
	}
	...
	if (rc < 0) {
		dev_err(chip->dev, "Cannot read SourceDet rc = %d\n", rc);
		return 0;
	}

	/* IF laptop USB or USB AC charger */
	if (reg & (SDP_BIT | CDP_BIT | DCP_BIT))
	{
		/* get USBIN status */
		rc = smb135x_read(chip, STATUS_8_REG, &reg);
		if (rc < 0) {
			dev_err(chip->dev, "Cannot read USBIN rc = %d\n", rc);
			return 0;
		}
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
	870,        /* current for index 0x0F -- Qualcomm doc 80-NF105-1,  register 0Ch (USBIN) */
#endif
/* NTGRSTOP */
	900,
	960,
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
  /* remove extra table entries, there are only 32 possible indexes */
#else
	1620,
	1680,
#endif
...
/*************
 *
 * Name:     smb135x_get_prop_input_curr_limit
 *
 * Purpose:  Get Charger's USB input current limit
 *
 * Parms:    (IN) chip  - pointer to driver structure
 *
 * Return:   milliamps
 ...
 *
 **************/
static int smb135x_get_prop_input_curr_limit(struct smb135x_chg *chip)
{
  int rc;
  u8  reg;
  u8  field;

  /* IF cannot read Charger mode */
  ...
  /* IF current limit is configured */
  field = reg & (USB_AC_VAL | USE_REGISTER_FOR_CURRENT);
  if ( field == (USB_AC_VAL | USE_REGISTER_FOR_CURRENT))
  {
    /* IF cannot read limit */
    rc = smb135x_read(chip, IN_LIMIT_STATUS_REG, &reg);
    if (rc < 0)
    {
      return 0;
    }
    ...
    rc = usb_current_table_smb1356[ reg ];
  }
  /* ELSE running in USB100 or USB500 mode */
  else
  {
    rc = (reg & USB_100_VAL) ? 100: 500;
  }
...
/* NTGRSTART */
	...
	/* IF no-battery in hotspot */
	if ( !smb135x_get_prop_batt_present(chip)) {
		/* cannot suspend or we'll die */
		suspend = false;
	}
#endif
/* NTGRSTOP */
...
/*************
 *
 * Name:     smb135x_set_output_usb_current
 *
 * Purpose:  Set output current limit
 *
 * Parms:    (IN) chip        - pointer to driver structure
 *           (IN) current_ma  - current limit (mA)
 *
 * Return:   0, if success
 *           Otherwise an error code 
 *
 **************/
static int smb135x_set_output_usb_current(struct smb135x_chg *chip,
							int current_ma)
{
	int i, rc;
	u8 usb_cur_val;
	...
	/* Write to the Fast-charge current limit register */
	rc = smb135x_masked_write(chip, CHARGE_LIMITS_REG,
					USBIN_INPUT_MASK, usb_cur_val);

	if (rc < 0)
		dev_err(chip->dev, "Couldn't write CHARGE_LIMITS_REG rc = %d\n", rc);
	return rc;
}
#endif
/* NTGRSTOP */
...
#ifdef CONFIG_NETGEAR
{
	...
	/* IF USB spec 500mA, use APSD results for current limit */
	cmdbit = (current_ma == CURRENT_500_MA) ? 0: USE_REGISTER_FOR_CURRENT;
	(void) smb135x_masked_write(chip, CMD_INPUT_LIMIT, USE_REGISTER_FOR_CURRENT, cmdbit);
}
#endif
/* NTGRSTOP */
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
	/* output limit is different than input limit */
	if (current_ma < INCURR_LIMIT_AC)
	{
		current_ma = OUTCURR_LIMIT_USB;
	}
	else if (current_ma == INCURR_LIMIT_AC)
	{
		/* normal 1 Amp wall adapter */
		current_ma = OUTCURR_LIMIT_AC;
	}
	else
	{
		/* AC-2amp or QuickCharge-5V or QuickCharge-9V wall adapter */
		current_ma = OUTCURR_LIMIT_AC_QCHARGE;
	}
	rc = smb135x_set_output_usb_current(chip, current_ma);
#endif
/* NTGRSTOP */
...
#ifdef CONFIG_NETGEAR
/*
 * Work-around to prevent an open charging FET.
 * See Qualcomm doc 80-NJ318-4, issue 11, "Vsys voltage drop when charging starts in full-on mode"
 */
	if (enable) {

		(void) smb135x_set_high_usb_chg_current( chip, INCURR_LIMIT_PRE_EN);
		/*
		 * Later, the (BX) Battery Manager will raise current limit to desired level
		 * because it always verifies Charger register IN_LIMIT_STATUS_REG.
		 */
	}
...
/*************
 *
 * Name:     smb135x_set_curr_termination
 *
 * Purpose:  Set charge termination current
 *
 * Parms:    (IN) chip  - pointer to driver structure
 *           (IN) iterm - termination current (mA)
 *
 * Return:   none
 *
 **************/
static void smb135x_set_curr_termination( struct smb135x_chg *chip, int iterm)
{
  int rc;
  u8 reg;

  /* map current to register field */
  if (iterm <= 50)
    reg = CHG_ITERM_50MA;
  else if (iterm <= 100)
    reg = CHG_ITERM_100MA;
  ...
  if (rc == 0)
  {
    ...
    if (rc != 0)
    {
      dev_err(chip->dev, "Cannot enable iterm rc = %d\n", rc);
    }
  }
  else
  {
    dev_err(chip->dev, "Cannot set iterm to %d: rc = %d\n", iterm, rc);
  }
}

/*************
 *
 * Name:     smb135x_set_charge_control_mask
 *
 * Purpose:  Edit collection of custom features via a mask
 *
 * Parms:    (IN) chip   - pointer to driver structure
 *           (IN) mask   - 32-bit mask that enables/disables custom features
 *
 *                         bit 0 = FET control (0=try to open FET, 1=charger manages)
 *                         bit 1 = Recharge level (0=normal level, 1=hot level)
 *                         bit 2 = QuickCharge (0=disabled, 1=enable)
 *                         bits 3-31 = undefined
 *
 * Return:   none
 *
 **************/
static void smb135x_set_charge_control_mask( struct smb135x_chg *chip, int mask)
{
	int  open;
	int  hot;
	int  qcharge;
	...
	/* enable FET override */
	smb135x_masked_write(chip, CFG_19_REG,
						BATT_FET_OVERRIDE_EN, BATT_FET_OVERRIDE_EN);

	/* FET control (open or managed by charger) */
	open = (mask & BYCUST_FET_MASK) ? false: true;
	smb135x_masked_write(chip, CMD_INPUT_LIMIT,
						FET_EDIT_MASK, (open) ? FET_OPEN_MASK: FET_CHARGER_MASK);

	/* Battery recharge level (hot is -200mV, normal is -50mV) */
	hot = (mask & BYCUST_RECHG_MASK) ? true: false;
	smb135x_masked_write(chip, CFG_5_REG,
						RECHARGE_200MV_BIT, (hot) ? RECHARGE_200MV_BIT: 0);

	/* QuickCharge (enable or disable) */
	qcharge = (mask & BYCUST_QCHARGE_MASK) ? true: false;
	smb135x_masked_write(chip, CFG_E_REG,
						QUICKCHARGE_MASK, (qcharge) ? QUICKCHARGE_ENABLE: 0);
}
#endif
/* NTGRSTOP */
...
/* NTGRSTART */
	...
	case POWER_SUPPLY_PROP_HEALTH:
		chip->prop_health_temp = val->intval;		/* select health status: temperature or fault */
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		smb135x_float_voltage_set( chip, val->intval);
		break;
	case POWER_SUPPLY_PROP_INPUT_CURRENT_TRIM:
		smb135x_set_curr_termination( chip, val->intval);
		break;
	case POWER_SUPPLY_PROP_USB_OTG:
		/* NTGR_TBD - 9x50 OTG is handled by other h/w, but leave sysfs path for now */
		rc = -EINVAL;
		break;
	case POWER_SUPPLY_PROP_INPUT_CURRENT_MAX:
		smb135x_set_usb_chg_current( chip, val->intval);
		break;
	case POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT:
		smb135x_set_charge_control_mask(chip, val->intval);
		break;
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		{
			/*
			 * Two ways of enabling the Charger are supported.
			 *   1 = Qualcomm way
			 *   2 = Qualcomm way + forced recharge
			 *       (Needed after a soft reset such as at!reset or FDT download so that
			 *        charging continues.  Otherwise charger waits for large voltage drop.)
			 */
			int enable = (val->intval) > 0 ? true: false;

			/* enable/disable */
			smb135x_charging(chip, enable);

			/* IF enabling + restarting */
			if (val->intval > 1)
			{
				/* kick recharge so it doesn't wait for a big voltage drop */
				force_rechg_set( chip, 0);
			}
		}
		break;
#else
...
#else
		if (chip->prop_health_temp)
		{
			/* Qualcomm returns Linux temperature-states */
			val->intval = smb135x_get_prop_batt_health(chip);
		}
		else
		{
			/* our function returns Linux fault-states */
			val->intval = smb135x_get_prop_charger_faults(chip);
		}
#endif
/* NTGRSTOP */
...
#ifdef CONFIG_NETGEAR
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		val->intval = smb135x_get_prop_volt_now(chip);
		break;
	case POWER_SUPPLY_PROP_INPUT_CURRENT_MAX:
		val->intval = smb135x_get_prop_input_curr_limit(chip);
		break;
	case POWER_SUPPLY_PROP_INPUT_CURRENT_TRIM:
		val->intval = smb135x_get_prop_curr_termination(chip);
		break;
	case POWER_SUPPLY_PROP_USB_OTG:
		val->intval = smb135x_get_otg_control(chip);
		break;
	case POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT:
		val->intval = smb135x_get_charge_control_mask(chip);
		break;
#endif
/* NTGRSTOP */
	default:
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
#define HIGH_RANGE_FLOAT_MIN_MV		4330
#else
...
#define VHIGH_RANGE_FLOAT_MIN_VAL	0x2D
#else
...
/*************
 *
 * Name:     smb135x_get_prop_volt_now
 *
 * Purpose:  Get Charger's output voltage
 *
 * Parms:    (IN) chip  - pointer to driver structure
 *
 * Return:   millivolts
...
static int smb135x_get_prop_volt_now(struct smb135x_chg *chip)
{
  ...
  /* IF cannot read Charger voltage */
  rc = smb135x_read(chip, FLOATV_STATUS_REG, &reg);
  if (rc < 0)
  {
    return 0;
  }
  ...
  /* IF minimum reported by Charger */
  if (reg <= FLOATV_MIN_STEP)
  {
    return FLOATV_MIN;
  }

  /* IF maximum reported by Charger */
  if (reg >= FLOATV_MAX_STEP)
  {
    return FLOATV_MAX;
  }

  /* IF voltage range has 0.01v increments */
  if ((reg >= HIGH_RANGE_FLOAT_MIN_VAL) &&
      (reg <  VHIGH_RANGE_FLOAT_MIN_VAL))
  {
    /* Charger has a few 0.01v increments in the middle of 0.02v increments */
    step = reg - HIGH_RANGE_FLOAT_MIN_VAL + 1;
    return (HIGH_RANGE_FLOAT_MIN_MV + (step * HIGH_RANGE_FLOAT_STEP_MV));
  }
  /* ELSE IF voltage range is "very high" and 0.02v increments */
  else if (reg >= VHIGH_RANGE_FLOAT_MIN_VAL)
  {
    step = reg - VHIGH_RANGE_FLOAT_MIN_VAL;
    return (VHIGH_RANGE_FLOAT_MIN_MV + (step * VHIGH_RANGE_FLOAT_STEP_MV));
  }
     
  /* lower voltage range */
  step = reg - FLOATV_MIN_STEP;
  return (FLOATV_MIN + (step * FLOATV_STEP));
}
#endif
/* NTGRSTOP */
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
		/* Always start with 50mV recharge. Netgear BX package will edit if battery is hot */
		reg = 0;
#else
...
/*
 * Work-around to prevent an open charging FET.
 * See Qualcomm doc 80-NJ318-4, issue 12, "Vsys voltage drop when OTG is disabled"
 *
 * 9x50 higher-layer code does not enable OTG on this charger,
 * but this work-around is applied in case future products use the charger's OTG.
 *
 * Note we've always used hardware revision 2.1 so the code above with
 * (chip->revision == REV_2) can be ignored.
 */
		(void) smb135x_masked_write(chip, CFG_18_REG, STANDBY_VCHG_MASK, 0);
 		/* Default for Standby VCHG is disabled so don't bother restoring */
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
	dev_err( chip->dev, "OTG enable controlled by other h/w (rc=%d)\n", rc);
#else
	chip->otg_oc_count = 0;
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
	dev_err( chip->dev, "OTG disable controlled by other h/w (rc=%d)\n", rc);
#else
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
	/* IF a foreign adapter */
	if (usb_supply_type == POWER_SUPPLY_TYPE_UNKNOWN)
	{
		/* identify as AC (bxtask will reduce current limit) */
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
#define SRC_DETECT_LOW_DELAY_MS		msecs_to_jiffies(500)
#define SRC_DETECT_LOW_TRIES		40
#else
...
/* NTGRSTART */
	...
	/* NOTE: this thread is shared between two USB interrupts <falling-edge and under-voltage> */

	/* get USB UnderVolt status */
	rc = smb135x_read( chip, IRQ_E_REG, &reg);
	if (rc < 0) {
		dev_err(chip->dev, "Cannot read IRQ_E_REG (%d)\n", rc);
		smb135x_relax(chip);
		return;
	}
	low_volt = (reg & IRQ_E_USB_UV_BIT) ? true: false;

	/* get USB Source status */
	rc = smb135x_read( chip, IRQ_G_REG, &reg);
	if (rc < 0) {
		dev_err(chip->dev, "Cannot read IRQ_G_REG (%d)\n", rc);
		smb135x_relax(chip);
		return;
	}
	src_present = (reg & IRQ_G_SRC_DETECT_BIT) ? true: false;

	/*
	 * Logic here is delayed from the original under-volt interrupt
	 * and we decide if the USB cable is actually removed.
	 *
	 *   usb_present, low_volt, src_present  =   ACTION
	 *   ---------------------------------------------------------------------
	 *       0            x         x            do nothing
	 *       1            0         0            do nothing (not possible ??)
	 *       1            0         1            do nothing (recovered from volt-sag)
	 *       1            1         0            Cable Removed (disconnect from USB/AC)
	 *       1            1         1            do nothing (volt-sag is long or many)
	 */

	/* IF cable "was" connected */
	if (chip->usb_present) {
		/* IF voltage still low AND no cable now */
		if (low_volt && !src_present) {
			chip->usb_present = false;
			handle_usb_removal(chip);  /* inform USB Protocol Stack */
		} else {
			/* re-schedule in case it takes longer */
			if (chip->src_detect_low_tries < SRC_DETECT_LOW_TRIES) {
				chip->src_detect_low_tries++;
				schedule_delayed_work( &chip->src_detect_low_work, SRC_DETECT_LOW_DELAY_MS);
				return;
			}
		}
	} /* IF cable "was" connected */
...
#ifdef CONFIG_NETGEAR
		/*
		 * First assume the Adapter is bad.
		 * If SMB Charger detects a sag after its glitch filter (reg 0x0D),
		 * it'll lockup until the USB cable is pulled.
		 *
		 * So we attempt to avoid lockup by reducing the current limit here.
		 */
		smb135x_masked_write( chip, CFG_D_REG, AICL_ENABLE, 0);
		smb135x_set_high_usb_chg_current( chip, INCURR_LIMIT_AC_SAG);
		/* keep AICL disabled so there's only 1 master controlling current limits when USB/AC voltage collapse */

		/*
		 * Cancel work thread src_detect_check_work().
		 * It's shared between two USB interrupts <falling-edge and under-voltage>
		 * There might be multiple sags or interrupts serviced in reverse order.
		 */
		cancel_delayed_work_sync( &chip->src_detect_low_work);

		/* Later, decide if Adapter is bad OR the USB cable is removed */
#endif
/* NTGRSTOP */
		smb135x_stay_awake(chip);
		schedule_delayed_work(&chip->src_detect_low_work,
...
/* NTGRSTART */
	 ...
	 */
		...
		/* Later, handle USB cable removal */
		smb135x_stay_awake( chip);
		schedule_delayed_work( &chip->src_detect_low_work, SRC_DETECT_LOW_DELAY_MS);
	}
#else
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
	chip->safety_time = 0; 		/* disable safety timer */
	chip->prop_health_temp = true;		/* health property: temperature-states */
	smb135x_path_suspend( chip, DC, DCIN_DISABLE, true); /* disable Wireless interface */
	smb135x_write( chip, CFG_B_REG, 0);                  /* ensure Wireless AICL is disabled */
#endif
/* NTGRSTOP */
...
/* NTGRSTART */
	...
	rc = smb135x_write(chip, CFG_JEITA_LIMITS_REG, JEITA_LIMITS);
	if (rc < 0) {
		dev_err(chip->dev, "Cannot set JEITA temperature limits: rc = %d\n",
				rc);
		goto free_regulator;
	}

	/*
	 * Default input and output current limits.
	 * Must start in USB500 mode so Charger's state machine uses APSD results for the current limit.
	 * After driver initialization, current limit can be increased.
	 */
	rc = smb135x_set_usb_chg_current( chip, INCURR_LIMIT_DEFAULT);
	if (rc < 0) {
		dev_err(chip->dev, "Cannot set default current limits: rc = %d\n",
				rc);
		goto free_regulator;
	}

	/* JEITA hot-soft limit charge current compensation */
	rc = smb135x_masked_write(chip, CFG_1A_REG, HOT_SOFT_LIMIT_MASK, HOT_SOFT_LIMIT_ENABLE);
	if (rc < 0) {
		dev_err(chip->dev, "Cannot enable hot soft limit charge current compensation: rc = %d\n",
				rc);
		goto free_regulator;
	}
	
	/* JEITA hot-hard charge current compensation */
	rc = smb135x_masked_write(chip, HOT_COMPENSATION_REG, HOT_CURRENT_LIMIT_MASK, HOT_CURRENT_LIMIT_250MA);
	if (rc < 0) {
		dev_err(chip->dev, "Cannot set fast charge current compensation: rc = %d\n",
				rc);
		goto free_regulator;
	}
	
/* ELSE Qualcomm JEITA temperature thresholds */
#else
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
	/* JEITA hot/cold compensation is defined by Netgear */
#else
	/*
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
	/* Always use 200V inhibit to match the h/w default */
