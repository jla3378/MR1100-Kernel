...
static const struct qpnp_vadc_map_pt ntgr_adcmap_batt_therm_threshold[] = {
	{-300, 1659},
	{-250, 1618},
	{-200, 1568},
	{-150, 1509},
...
/* Units for temperature below (on y axis) is in 0.1 DegC as
   required by the PA therm driver. Note the resolution used
   here to compute the table was done for DegC to milli-volts.
   In consideration to limit the size of the table for the given
   temperature range below, the result is linearly interpolated
   and provided to the battery driver in the units desired for
   their framework which is 1.0 DegC. */
static const struct qpnp_vadc_map_pt ntgr_adcmap_pa_therm_threshold[] = {
	{1734, -300},
	{1707, -250},
	{1672, -200},
	{1627, -150},
...
/* Units for temperature below (on y axis) is in 0.1 DegC as
   required by the board therm driver. Note the resolution used
   here to compute the table was done for DegC to milli-volts.
   In consideration to limit the size of the table for the given
   temperature range below, the result is linearly interpolated
   and provided to the battery driver in the units desired for
   their framework which is 1.0 DegC. */
static const struct qpnp_vadc_map_pt ntgr_adcmap_board_therm_threshold[] = {
	{1734, -300},
	{1707, -250},
	{1672, -200},
	{1627, -150},
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
	/* ((ADC code - ADC gnd) * 1875mV) / 0x4000 */ 
	bat_voltage = (int64_t) ((adc_code - chan_properties->adc_graph[CALIB_RATIOMETRIC].adc_gnd) 
		* adc_properties->adc_vdd_reference);

	bat_voltage = div64_s64(bat_voltage, QPNP_VADC_HC_VREF_CODE);
#else
	qpnp_adc_scale_with_calib_param(adc_code,
...
/* NTGRSTART */
	...
	return qpnp_adc_map_temp_voltage(
			ntgr_adcmap_batt_therm_threshold,
			ARRAY_SIZE(ntgr_adcmap_batt_therm_threshold),
			bat_voltage,
...
#ifdef CONFIG_NETGEAR
	qpnp_adc_map_voltage_temp(ntgr_adcmap_board_therm_threshold,
		ARRAY_SIZE(ntgr_adcmap_board_therm_threshold),
...
/* NTGRSTART */
#ifdef CONFIG_NETGEAR
		qpnp_adc_map_voltage_temp(ntgr_adcmap_pa_therm_threshold,
			ARRAY_SIZE(ntgr_adcmap_pa_therm_threshold),
			therm_voltage, &adc_chan_result->physical);
#else
		qpnp_adc_map_voltage_temp(adcmap_100k_104ef_104fb_1875_vref,
			ARRAY_SIZE(adcmap_100k_104ef_104fb_1875_vref),
