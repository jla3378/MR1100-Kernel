...
#ifdef CONFIG_NETGEAR
/*---Engg code --*/ 
/* The following code is added for MDM9x50 so that
 * the GPIO 3 (amber LED) is completely off 
 * when the device shuts down (QTI case 3155139)
 */
	engg_reg_writes = 0xC246; 
	rc = qpnp_pon_masked_write(pon, engg_reg_writes, 0xFF,0x00); 
	if (rc) 
		dev_err(&pon->spmi->dev, 
			"Unable to write to addr=%hx, rc(%d)\n", 
			engg_reg_writes, rc); 

	engg_reg_writes = 0xC242;	
	rc = qpnp_pon_masked_write(pon, engg_reg_writes, 0xFF,0x00); 
	if (rc) 
		dev_err(&pon->spmi->dev, 
			"Unable to write to addr=%hx, rc(%d)\n", 
	...
	engg_reg_writes = 0xC2D0;	
	rc = qpnp_pon_masked_write(pon, engg_reg_writes, 0xFF,0xA5); 
	if (rc) 
		dev_err(&pon->spmi->dev, 
			"Unable to write to addr=%hx, rc(%d)\n", 
			engg_reg_writes, rc); 

	engg_reg_writes = 0xC2DA;	
	rc = qpnp_pon_masked_write(pon, engg_reg_writes, 0xFF,0x08); 
	if (rc) 
		dev_err(&pon->spmi->dev, 
		"Unable to write to addr=%hx, rc(%d)\n", 
		engg_reg_writes, rc); 
