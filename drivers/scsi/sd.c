...
/**
 *	sd_test_unit_ready
 *	performs test unit ready
 **/
static int sd_test_unit_ready(struct scsi_disk *sdkp)
{
	static unsigned char cmdtest[10];
	static struct scsi_sense_hdr sshdr;
	struct scsi_device *sdp = sdkp->device;
	int res;

	cmdtest[0] = TEST_UNIT_READY;
	memset((void *) &cmdtest[1], 0, 9);

	res = scsi_execute_req_flags(sdp, cmdtest,
			DMA_NONE, NULL, 0,
			&sshdr, SD_TIMEOUT,
			SD_MAX_RETRIES, NULL, REQ_PM);
	if (res) {
		sd_printk(KERN_WARNING, sdkp, "Test Unit Ready Error\n");
		sd_print_result(sdkp, res);
		if (driver_byte(res) & DRIVER_SENSE)
			sd_print_sense_hdr(sdkp, &sshdr);
		if (scsi_sense_valid(&sshdr) &&
